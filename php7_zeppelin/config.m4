...
if test "$PHP_ZEPPELIN" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $ZEPPELIN_DIR/$PHP_LIBDIR, ZEPPELIN_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_ZEPPELINLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong zeppelin lib version or lib not found])
  dnl ],[
  dnl   -L$ZEPPELIN_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(ZEPPELIN_SHARED_LIBADD)

  PHP_NEW_EXTENSION(zeppelin, zeppelin.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
